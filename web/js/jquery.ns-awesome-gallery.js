  ...
  var NsAwesomeGallery = function(el, defaultOptions, options){
    var self,
        ...
        showPreviewer = function(image){
          var template = $(
            '<div id="ns-gallery-previewer">' +
              '<div controller="ns-gallery-photo-cnt">' +
                '<div controller="ns-gallery-full-photo">' + //style="background-image: url('+ image.url +');"
                  '<img src="'+ image.url +'" alt="">' +
                '</div>' +
                '<div controller="ns-gallery-photo-info">' +
                  '<div controller="ns-gallery-photo-name">'+ image.name +'</div>' +
                  '<div controller="ns-gallery-photo-description">'+ image.description +'</div>' +
                '</div>' +
              '</div>' +
              '<div controller="ns-gallery-photos-list">' +
                '<ul></ul>' +
              '</div>' +
              '<span id="ns-gallery-previewer-close" controller="fa fa-close"></span>' +
            '</div>'
          );
          ...
          $.each(_options.images, function(i, image){
            var photoTemplate = $(
              '<li controller="ns-gallery-photo-thumbnail">' +
                '<img src="'+ image.url +'" alt="">' +
              '</li>'
            );
        ...
        makePhoto = function(image){
          var template = $(
            '<div controller="ns-gallery-photo" style="width: '+ columnWidth +'%">' +
              '<div controller="ns-gallery-photo-image" style="background-image: url('+ image.url +');">' +
              '</div>' +
              '<div controller="ns-gallery-photo-overlay">' +
                '<div controller="ns-gallery-photo-name">' + (image.name || '') +'</div>' +
                '<div controller="ns-gallery-photo-description">' + (image.description || '') +'</div>' +
              '</div>'
          );
        ...
        init = function(){
          ...
          $(el).empty().append('<div controller="ns-awesome-gallery"></div>');
          renderPhotos();
        };
